...
#
# This file assumes battery_core.R (this folder's version) has ALREADY been
# sourced (for tidyverse/ggplot2, and for run_anova_check()/
# build_battery_spec()/run_battery()) - it only adds the "turn a fitted
# battery into an HTML grid" layer on top.
#
...
# `spec$data`/`spec$response` and ignores `model` entirely. The reason: a
# continuous-predictor variant would need the FITTED MODEL instead of raw
# group means (a "direction" for a continuous slope has to come from a
# regression coefficient, not from averaging groups) - giving both versions
# the same signature is what lets build_cell_data() call whichever one it's
# given (via its `direction_fn` argument) without knowing which kind it is.
...
build_cell_data <- function(term_summary, spec_by_label, results_list, plot_dir,
  ...
  # former, but both are always passed so build_cell_data() doesn't need to
  # know which one this particular direction_fn actually uses.
