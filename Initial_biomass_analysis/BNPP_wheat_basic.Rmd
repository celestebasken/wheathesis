...
ggplot(BNPP_strl, aes(x = Water, y = Biomass_g_pp)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.15, alpha = 0.6) +
  facet_wrap(~ Polyculture) +
  ggtitle("BNPP biomass by PPT, Sterile treatments only")

# visualizing this with water as continuous

BNPP_sum_strl <- BNPP_strl %>%
  group_by(Inoculation, Polyculture, Water_gal_per_plant_wk) %>%
  summarise(mean_biomass = mean(Biomass_g_pp, na.rm = TRUE), .groups = "drop")
...
ggplot(
  ...
  geom_line() +
  geom_point() +
  ggtitle("BNPP biomass by PPT (gallons per week) of sterile treatments", subtitle = "With precipitation adjusted to per-plant density") +
  xlab("Gallons of water per week, density-adjusted") +
  ylab("Mean BNPP (g)")
